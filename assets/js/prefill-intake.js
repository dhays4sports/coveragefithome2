(() => {
  ...
  const PII_KEYS = [
    'first_name', 'last_name', 'phone', 'email', 'property_address', 'segment',
  ...
  const MARKER_KEYS = ['prefill', 'handoff_version'];
  ...
  const profile = {
    ...
    propertyAddress,
    reviewContext: clean(params.get('segment'), 120),
    address: {
    ...
    integration: {
      source: clean(params.get('source'), 80) || '408farmers',
