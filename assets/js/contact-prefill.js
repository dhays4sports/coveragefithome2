(() => {
  ...
  const apply = () => {
    ...
    if (!profile) {
      window.CoverageFitContactPrefill = { applied: false, fields: [], profile: null };
      return;
    }

    const address = profile.address || {};
    const values = {
      firstName: clean(profile.fullName || [profile.firstName, profile.lastName].filter(Boolean).join(' '), 160),
      email: clean(profile.email, 254).toLowerCase(),
      phone: clean(profile.phone, 40),
      propertyField: clean(address.postalCode, 20)
    };
