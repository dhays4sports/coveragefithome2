(function (root, factory) {
  ...
  function buildNextSteps(printModel, priorities) {
    const steps = priorities.map(priority => `Review ${priority} together.`);
    if (printModel?.consultationChecklist?.available) steps.push('Complete the remaining consultation checklist items.');
    if (printModel?.timeline?.state === 'ready') steps.push('Use the consultation timeline to guide the discussion.');
