(async()=>{
 ...
 const score=Math.max(0,Math.min(100,100+points-(missing.length*2)));
 const rating=score>=85?'Well Prepared':score>=70?'Strong Foundation':score>=50?'Review Recommended':'Several Areas to Review';
 const risks=a.filter(x=>Number(x.points||0)<0).sort((x,y)=>Number(x.points)-Number(y.points));
 const strengths=a.filter(x=>Number(x.points||0)>=0 && !['current_carrier','renewal_date'].includes(x.key));
