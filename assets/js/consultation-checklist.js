(function (root, factory) {
  ...
  function createItem(input, context) {
    ...
    return {
      ...
      metadata: clone(source.metadata) || { type: text(source.type, 'conversation-step') }
    };
  }
  ...
  function planFingerprint(plan, phases, items) {
    const payload = {
      ...
      items: items.map(item => [item.id, item.phaseId, item.title, item.estimatedMinutes, item.recommendationIds])
    };
