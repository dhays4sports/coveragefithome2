(function (root, factory) {
  ...
  const SECTION_CONTRACTS = deepFreeze({
    ...
    propertySummary: {
      ...
      optionalFields: ['yearBuilt', 'livingArea', 'stories', 'construction', 'roof', 'foundation', 'pool', 'detachedStructures', 'confirmation', 'quality']
    },
    recommendations: {
      type: 'array',
      required: true,
  ...
  function normalizeRecommendations(recommendations) {
    return asArray(recommendations).map((item, index) => ({
      ...
      summary: text(item?.summary || item?.description || item?.why),
      question: text(item?.question || item?.prompt),
    ...
    return {
      ...
      sections: asArray(source.sections).map(section => ({
        ...
        items: asArray(section?.items).map(item => ({
          ...
          coachingNote: text(item?.coachingNote), sourceIds: asArray(item?.sourceIds).map(String)
        }))
      })),
